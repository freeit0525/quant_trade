# -*- coding: utf-8 -*-
"""K 线补齐任务监督进程：工作进程被看门狗（单只拉取超时）强制退出后自动重启续跑

用法（透传 _backfill_kline_batch.py 的全部参数）：
  python _backfill_kline_supervisor.py --batch-size 100 --pause 30 --sleep 0.8 --watchdog-sec 480
...
def main():
    ...
    restart_count = 0
    while True:
        cmd = [sys.executable, "_backfill_kline_batch.py"] + args
